...
export default function CartSidebar() {
  ...
  useEffect(() => {
    // prevent background page scrolling when cart/order/success is open
    if (cartOpen || showOrderForm || successOrder) {
      const previous = document.body.style.overflow;
      document.body.style.overflow = 'hidden';
      return () => { document.body.style.overflow = previous; };
    }
    return undefined;
  }, [cartOpen, showOrderForm, successOrder]);
  ...
  return (
    <>
      ...
      {/* Cart Sidebar */}
      {!showOrderForm && !successOrder && (
        <div className={`fixed right-0 top-0 h-full w-full sm:w-[420px] bg-white z-50 cart-sidebar flex flex-col shadow-2xl   ${cartOpen ? 'translate-x-0' : 'translate-x-full'
          }`}>
          {/* Header */}
